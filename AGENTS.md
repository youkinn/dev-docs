...
4. 通过 → 成员发起 Pull Request（目标 main；标题沿用 commit 规范，描述注明需求编号与 Coco 审查结论）
5. 负责人二次审核（业务：需求符合度、是否上线、对外影响），通过 → 合并 PR
6. 负责人打回 → 成员在同一分支修正推送 → Coco 复审新改动 → 回到 5
7. 合并完成后 Coco 更新需求文档「合并记录」表（审查结果 + 合并日期）
