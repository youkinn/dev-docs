...
- **多人协作先拉需求集成分支（2026-09-14 决策）。** 同一项目内多人开发同一需求时：Coco 先基于 `origin/main` 拉本次需求集成分支 `coco/feat-XXX_{描述}`；成员基于该集成分支拉个人分支开发（单人需求仍直接基于 `origin/main` 拉分支）；**往集成分支合入不经过 PR，Coco 直接合并**，集成测试通过后，集成分支整体发 PR 合入 `main`（负责人执行）。避免未经集成测试的代码直接进主分支
...
4. 通过 → 单人需求：成员发起 PR 目标 main；多人协作：Coco 将成员分支直接合入集成分支并推送（往集成分支不经过 PR，直接合并）
5. 多人协作：成员改动全数合入集成分支后，Coco 跑集成测试（端到端全链路），通过后发起集成分支 → main 的 PR（集成分支进 main 必须走 PR，负责人合并）；单人需求直接进入 6
