...
- 工具明细 stage 与 llm_call_logs.stage **同名不同义**：前者指「该工具调用由哪个阶段驱动发起」（classify = 分类轮判定后按编号预调，generation = 生成轮的模型自主调用），后者指「本次 LLM 调用属于哪个阶段」；工具明细展示口径为「分类轮预调」/「生成轮调用」，避免同词误读（2026-09-22 负责人确认）
