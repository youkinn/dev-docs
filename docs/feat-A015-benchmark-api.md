...
- 汇总表列：类别 / 总题数 / 通过数 / 失败数 / 通过率 / Hit@5 命中数 / Hit@5 命中率；口径：通过数 = 命中数 = `summary.top5`、失败数 = `summary.tail + summary.miss`、通过率 = `top5 / total`。
- 「执行」按钮：点击调 `POST run`，执行中禁用并展示等待态，异常展示 `message`。
- 类别行可展开：该类别每题明细（编号 / 问题 / 标准答案 / 证据 / rank / 状态）+ 该题候选列表（点候选 chunkId 打开 `SangoChapterReader` 定位原文核对）。
