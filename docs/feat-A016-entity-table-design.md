...
合并示例（终审改键）：
- 关羽行：`关将军` / `汉寿亭侯` 终审多主条目候选 → fragmentOnly（R2）；`关公` 同判据原为 fragmentOnly，bug-00041 纠偏改回 rewriteKey（519 处 topPid=P002 share=1，P001/P005 为结义场景上下文共现非指称）；`美髯公` 唯一指称候选、`云长` / `关云长` / `关某` / `关二爷` / `赤面长须` 无终审记录 → 保留（R1 / R3）。
...
> 裁定说明：share=1 但 verdict 为多主条目候选的词仍按终审判定禁入改写键——终审为「初审分级 + 同串跨人组 + 指称份额」联合裁决，合并轮不自行重算、不以 share 单判覆盖 verdict。例外：`关公`（topPid=P002 share=1，P001/P005 为上下文共现非指称）经 bug-00041 复核判为误判、改回 rewriteKey；同批 R2 疑点（关将军 / 汉寿亭侯 / 刘使君 / 虎威将军 / 小霸王）待负责人定是否整体重审。
...
- 2026-09-26：bug-00041 修订——§3 合并示例关羽行 `关公` 由 fragmentOnly 纠偏为 rewriteKey（终审 share=1 误判，证据见 bug 票）；§4 裁定说明补例外与同批疑点清单（Coco 定案，老陈实施数据）。
