...
- **Bug1 快路径（第九轮，已落地推送 6105cc3）**：`domain=sango-novel` 预调 sango_novel_search（limit 5）→ 注入原文片段到 user 消息 → 单次 LLM 生成，可用工具移除检索工具防重复调；响应 7~17s → 目标 1~2s（实测仍 7~17s，受火山无缓存 + 注入片段过长影响，待火山缓存 + 上下文收窄）
...
- mcp-orchestrator：`coco/feat-A004_sango-classics-rag`=`6105cc3`（第九轮：Bug1 快路径 + Bug2 固定格式 + 引用校验本地化去 NER，**已推送**）
...
- 测试：orchestrator 全量 99/99 绿（`node --test "build/test/*/*.test.js"`；删 3 个孤儿 NER 测试后 102→99）
...
- **上下文收窄工作单（小胡，待分配）**：`agent.ts` 快路径注入量收窄——limit 5→top3、注入片段截为检索词附近窗口（LLM 输入 2500→500 字）；产出：改 agent.ts + 测试 + 更新接口/技术方案/交接三份文档；预期火山缓存开通后 7~17s → 1.8~3.2s（实测）
