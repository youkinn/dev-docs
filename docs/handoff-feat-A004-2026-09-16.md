# 上下文交接：feat-A004 三国演义解读（2026-09-16 第八轮，待负责人验收）
...
- 生成约束三件套：prompt 5 条 + 引用硬校验（别名表 ID 优先，关羽→P002，v1 只校验人名）+ 兜底（原文片段+出处+结论句）
...
- mcp-orchestrator：`coco/feat-A004_sango-classics-rag`=`718264a`（第八轮：移除旧兼容 + npm run dev + README 通用流程，**已推送**）
...
- 测试：orchestrator 全量 102/102 绿（`node --test "build/test/*/*.test.js"`）
