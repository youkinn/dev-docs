 ...
 * 落点：待随 bug-00003 落成 mcp-orchestrator/scripts/probe/recall-bench.mjs
 *
 ...
 *   3. 注入窗口二阶截断率（复刻 citation.ts trimFragmentToWindow）
 */
...
idx.docs.forEach((d, i) => docKey.set(`${d.chapter}:${d.segIndex}`, i));
...
// --- V0：现状 search()（BM25 + 哈希向量混合），把格式化输出解析回段下标 ---
function ranksFromSearch(query, limit) {
  const out = idx.search(query, limit);
  if (!out || out.includes('未召回到')) return [];
  const ranks = [];
  for (const block of out.split('\n\n')) {
    const m = block.match(/^【出处】第(\d+)回.*?段(\d+)（/);
    if (m) {
      const k = docKey.get(`${m[1]}:${m[2]}`);
...
// --- 注入窗口二阶截断率（复刻 citation.ts trimFragmentToWindow） ---
