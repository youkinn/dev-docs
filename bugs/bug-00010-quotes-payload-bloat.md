...
> Bug 号：bug-00010
> 状态：**已修复**（2026-09-21 负责人验收通过并合并上线）
...
**验收（2026-09-21 负责人）**：通过。用户可见答案与引用未变，出参体积回落目标达成。

**合并（2026-09-21）**：`mcp-server` PR #9 / `mcp-orchestrator` PR #14 / `dev-docs` PR #22 已合并；两代码仓**同批上线**（编排侧硬依赖出参 `len`，不可单独上线）。
