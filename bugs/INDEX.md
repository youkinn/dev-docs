...
| bug-00019 | auto 路径每条问句先白跑一次 L3 题库预检（`fengyunsanguo_quiz_route` 返回 false，与 A011 轻量分类轮职责重叠）；日志页表现为一条问句两条工具调用。修复候选＝auto 统一「分类轮 → 按编号预调域工具」，移除 L3 重叠层 | mcp-orchestrator | feat-A011 | 待修复 | — |
