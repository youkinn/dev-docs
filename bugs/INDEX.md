...
| bug-00015 | 原文阅读器弹框点「上一回 / 下一回」抖动。根因＝`goTo()` 置 `data = null` 后正文区塌缩成 spinner，滚动容器高度与弹框尺寸跳变、滚动位置重置。修复口径＝滚动容器高度固定占满视口 + 正文区 scrollbar-gutter: stable（滚动条出现 / 消失不改纸宽），翻回不塌缩（feat-A010 验收打回） | mcp-web | feat-A010 | 已修复 | 2026-09-22 |
| bug-00016 | 原文阅读器打开后未滚动到指定片段（日志页候选分数表 chunkId 入口；二次验收仍未通过）。根因＝定位时机早于弹框正文挂载，`scrollRef` 为 null 即静默 return。修复口径＝等「容器已挂载 + 目标行存在」再测量（声明式 watch(flush: post)，不做无界轮询；注意 antd-vue 4.2.6 Modal 无 afterOpenChange，只有 afterClose） | mcp-web | feat-A010 | 已修复 | 2026-09-22 |
| bug-00017 | 日志页候选分数表点 chunkId 开 / 关阅读器弹框，页面抖一次且关闭后滚动位置被重置到顶部（验收 7a 相关）。根因＝antd 滚动锁改写 `document.body` 的 overflow + width，与 `scrollbar-gutter: stable` 槽位叠加。修复口径＝开 / 关不得改变页面宽度与滚动位置 —— 弹框 :get-container 指向非 body 的 #app（antd PortalWrapper 仅容器为 body 时加锁，不再改写 body 的 overflow / width），页面不滚动改由 wrap 与 .reader-scroll 的 overscroll-behavior: contain 保证 | mcp-web | feat-A010 | 已修复 | 2026-09-22 |
