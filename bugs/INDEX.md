...
| bug-00012 | A003 测试 harness 未注入独立日志库 → 测试写入开发库 data/logs.db，日志页出现测试噪声（「随机一题」既有「对话」又有「答题」；负责人核对：该条无耗时、无域，确认为测试数据）。修复口径＝两个 harness 注入 `:memory:`；存量噪声 2026-09-26 已按特征清理（request_logs 6326→2146，备份缺失）；两个 harness 现工作区（mcp-orchestrator / mcp-web / bench-mock / bench-verify）未定位到文件，`:memory:` 注入落地受阻 | mcp-orchestrator | feat-A007 / feat-A008 | 待修复 | — |
...
| bug-00032 | A016 验收：问「孙权遣人向关羽求亲，关羽是怎么回复使者的」答「演义中未涉及」（trace c549084b）。根因（2026-09-26 排查定案）=模型原始输出正确「关羽怒斥使者，拒绝联姻。[片段5]」（responseSummary 实证，指针=0073:c0008 求亲被拒原文、人物校验过），但 bug-00028 结构门第三条「句-片段重叠门」（0.5 @ 2-gram）对改述结论句整句裁剪→无留存句→拒答；字面 n-gram 无法区分「语义支撑的改述」与「先验断言」（两类重叠都低），测试套件缺「改述正例」漏网。修复口径=重叠门改「低重叠待裁→边界 LLM 语义支撑复核」（非全量二次调用）；回归补 3 条改述正例；关联 bug-00005 指针通道同族 | mcp-orchestrator | feat-A016 | 待修复 | — |
| bug-00033 | A016 验收：问「孙刘联军大破曹操的战役是哪一场」答「演义中未涉及」（trace ca7d9c6a，现象补录）。根因（2026-09-26 排查定案）=模型原始输出正确「孙刘联军大破曹操的战役是赤壁之战。[片段2]」，指针=0077:c0009（含「破曹操于赤壁」），重叠 0.2 被重叠门裁掉→拒答；次级暴露=ch49 火攻叙述 0049:c0018-20 未进 top20 候选，引用只能落间接提及段，属检索侧后续优化、不阻塞。修复口径=同 bug-00032 护栏语义复核 | mcp-orchestrator, mcp-server | feat-A016 | 待修复 | — |
| bug-00034 | A016 验收：同题跨请求答案不稳定——「夏侯惇的眼睛是怎么瞎的」67119582 答「夏侯惇左目为曹性射瞎」vs 275551c3 答「演义中未涉」。根因（2026-09-26 排查定案）=两次检索 / funnel / candidates 完全一致、prompt tokens 均 2238，仅模型措辞不同：左目为…射瞎（重叠 0.556 过线）vs 左眼被…射瞎（重叠 0.3 被裁）→ 重叠门对同义措辞（左目 / 左眼，恰为 A016 归一化范畴）一票否决；两 trace 原始输出均含 [片段1]=0018:c0014 正确段。修复口径=同 bug-00032（语义复核替代字面一票否决），判据与 A016 归一表联动 | mcp-orchestrator | feat-A016 | 待修复 | — |
