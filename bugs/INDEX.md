...
| bug-00016 | 原文阅读器打开后未滚动到指定片段（日志页候选分数表 chunkId 入口）。2026-09-22 二次验收仍未通过：弹框停在正文顶部。根因复核＝`load()` 里「先 `loading = false` + `await nextTick()`」只保证本组件渲染队列刷完，不保证 antd Modal 正文已挂载，`scrollRef` 仍可能为 null → `scrollToTarget()` 里 `if (!container) return` 静默放弃定位（无重试、无反馈）。修复口径＝等弹框正文真正挂载后再测量（`a-modal` 的 `@after-open-change` 或轮询到目标行存在，必要时重试一次），容器 / 目标行缺失不得静默 return（feat-A010 验收打回） | mcp-web | feat-A010 | 修复中 | — |
| bug-00017 | 日志页候选分数表点 chunkId 打开阅读器弹框后，开 / 关弹框各抖一次，且关闭后页面滚动位置被重置到页面顶部（feat-A010 二次验收打回，与验收 7a「打开时页面不滚动」同源）。根因待复核＝antd 滚动锁（`vc-util/Dom/scrollLocker.js`）在开 / 关时改写 `document.body` 的 `overflow: hidden` + `width: calc(100% - scrollBarSize)`，而 `html { overflow-y: auto; scrollbar-gutter: stable }` 已预留槽位 → 页面内容宽度被多减一档（居中容器横向位移＝抖动）、body 溢出状态被改写（页面滚动位置归 0）。修复口径＝弹框开 / 关不得改变页面内容宽度与 `document.scrollingElement.scrollTop`（保存 / 恢复 scrollTop 或绕开 body 级补偿），且不回退 7a 与 bug-00011（横向不抖动） | mcp-web | feat-A010 | 修复中 | — |
