...
| bug-00028 | 演义域片段不足以支撑结论仍凭先验作答（「马超投靠刘备后」答案对但注入片段无五虎/病逝、「夏侯渊字什么」原文无此信息却答妙才、「刘备死的时候多少岁」同族——提示词含禁用先验约束但模型不服从 + 引用不支撑结论）。当前实现：单轮生成 + 结构保险丝（复核轮已撤销）；负责人 2026-09-24 定：保持未修复状态。修复代码已就绪于分支 hu/bug-00028_answer-support-guardrail（+4 提交未合入 main、未提测）；现状见 docs/llm-answer-accuracy-current-state.md §11/§12 | mcp-orchestrator, mcp-server | feat-A004 | 待修复 | — |
