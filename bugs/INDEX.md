...
| bug-00010 | 演义检索工具出参 quotes[] 重复携带原文子串，响应 / 落库日志体积激增 | mcp-server, mcp-orchestrator | feat-A004 | 修复中 | — |
| bug-00011 | 首页 / weather / logs 互相切换时页面抖动（顶部宽度不一致），logs 两个标签间切换同样抖动 | mcp-web | feat-A007 | 已修复 | 2026-09-21 |
| bug-00012 | A003 测试 harness 未注入独立日志库 → 测试写入开发库 data/logs.db（日志页出现测试噪声：「随机一题」既有「对话」又有「答题」；修复＝harness 注入 `:memory:`） | mcp-orchestrator | feat-A007 / feat-A008 | 待修复 | — |
