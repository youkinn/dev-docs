...
| bug-00019 | 工具调用记录缺「调用方 / 发起阶段」（日志页只看得到「谁被调用」，看不出「谁发起、哪个阶段发起」）→ auto 路径的服务端 L3 题库预检被读成模型乱调工具（trace `2ef3608a`）。修复口径＝`tool_call_logs` 增 `caller`（model / server）+ `stage`（l3 / fastpath / classify / generation），历史 NULL 不回填；日志页「调用方法」后增「调用方」列合成显示 | mcp-orchestrator, mcp-web | feat-A011 | 修复中 | — |
