...
| bug-00032 | A016 验收：问「孙权遣人向关羽求亲，关羽是怎么回复使者的」LLM 直答正确（关羽怒斥使者，拒绝联姻[片段5]）但 /api/chat POST 返回「演义中未涉及」（trace c549084b）。根因待查（接口链路与直答不一致）；修复口径待定 | mcp-orchestrator, mcp-server | feat-A016 | 待修复 | — |
| bug-00033 | A016 验收：问「孙刘联军大破曹操的战役是哪一场」现象未记录（仅 trace ca7d9c6a），待复现补录 | mcp-orchestrator, mcp-server | feat-A016 | 待修复 | — |
| bug-00034 | A016 验收：同题跨请求答案不稳定——「夏侯惇的眼睛是怎么瞎的」trace 67119582 答「夏侯惇左目为曹性射瞎」vs trace 275551c3 答「演义中未涉」且 rank1 候选未被采纳（chunkId sanguo-yanyi:0018:c0014）。根因待查（同题不同 trace 结果不一致、候选已召回未进答案）；修复口径待定 | mcp-server, mcp-orchestrator | feat-A016 | 待修复 | — |
