...
| bug-00032 | A016 验收发现：演义域同题跨请求答案不稳定、与 LLM 直答不一致（「夏侯惇的眼睛是怎么瞎的」trace 67119582 答「左目为曹性射瞎」vs trace 275551c3 答「演义中未涉」且 rank1 候选未被采纳 chunkId sanguo-yanyi:0018:c0014；「孙权遣人向关羽求亲」trace c549084b LLM 直答正确但 /api/chat POST 返回「演义中未涉及」；「孙刘联军大破曹操的战役是哪一场」trace ca7d9c6a）。根因要点＝同题不同 trace 结果不一致、候选已召回却未进答案；修复口径待定（对齐 trace 定位检索 / 采纳链） | mcp-server, mcp-orchestrator | feat-A016 | 待修复 | — |
