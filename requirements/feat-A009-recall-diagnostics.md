...
> **验收通过（2026-09-22 负责人）：12 项验收标准全部勾选（含 6a / 6b 追加项）→ Coco 发起三侧 PR；PR 合并后置「已归档」**
> 追加（2026-09-21 负责人验收期提出，2026-09-22 已实现并验收通过）：标签命中列需可见「命中了哪个标签」→ 契约 `candidates[]` / `nextRank` 增 `hitLabels: string[]`，前端 hover 展示；验收见 6a
> 追加（2026-09-21 负责人验收期提出，2026-09-22 已实现并验收通过）：**最终分列 hover 展示算式代入过程**（原样列出算式 + 逐项代入实际值，不只给结果）；纯前端展示项，契约字段（`bm25Norm` / `cosine` / `labelHit` / `finalScore`）已由 bug-00013 补齐，无接口变更、无依赖阻塞；验收见 6b
> **排期（2026-09-21 负责人指示）：先修 bug（独立票独立分支，不并入本特性），bug 修完再开本特性开发；两者都完成后一起提测**
> 本批次 PR 一并携带 bug-00013（finalScore 可复算，三侧）与 bug-00014（第 81 回回目分隔符，mcp-server）修复——开发阶段（未上线）多 bug 同分支堆积，随批次合并（2026-09-21 决策）
...
6b. [√] **最终分列 hover 展示算式代入过程**（2026-09-21 负责人追加）：浮层原样列出算式 `finalScore = round3( 0.3 × BM25归一化 + 0.6 × 向量映射((cosine+1)/2) + 0.1 × 标签命中 )`，并**逐项代入实际值**（`bm25Norm`、`cosine` 与映射后的向量值、标签命中 `1|0`）→ 给出三个乘积、求和、`round3` 结果；**禁止只给最终数字**，过程必须在浮层里看得见（目的：免除手算）。`bm25Norm` / `cosine` 为 null 时按 `0` 代入并标注原因（非词法命中 / 降级纯 BM25 无向量分）。代入值展示 4 位小数（与列内口径一致），乘积与求和按全精度计算，最后一步 `round3` 显式写出，保证目视可复算。第 N+1 名卡片同口径
...
12. [√] 既有 A003~A008 相关测试全绿（诊断不改业务行为）（2026-09-22 抽样）
...
| story-A009-03 | 实现：sango 诊断产出 + 总台透传 / 落库 / 查询 + 编排侧诊断关联 | 老陈 + 小胡 | 已提测（2026-09-21）；验收通过（2026-09-22） |
| story-A009-04 | 前端：日志页检索诊断面板（含 6a 标签明细、6b 最终分算式 tooltip 追加项） | 小叶 | 6a 已交付（2026-09-21）；6b 已交付（2026-09-22，复用本故事号）；均验收通过 |
