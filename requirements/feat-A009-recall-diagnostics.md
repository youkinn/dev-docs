...
> 追加（2026-09-21 负责人验收期提出，待实现后重新提测）：**最终分列 hover 展示算式代入过程**（原样列出算式 + 逐项代入实际值，不只给结果）；纯前端展示项，契约字段（`bm25Norm` / `cosine` / `labelHit` / `finalScore`）已由 bug-00013 补齐，无接口变更、无依赖阻塞；验收见 6b
...
| story-A009-04 | 前端：日志页检索诊断面板（含 6a 标签明细、6b 最终分算式 tooltip 追加项） | 小叶 | 6a 已交付（2026-09-21）；6b 追加项开发中（2026-09-21，复用本故事号） |
