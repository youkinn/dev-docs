...
> 特性号：feat-A009
> 状态：已提测（2026-09-21 Coco 提测；审查一轮打回已修复：补硬约束 2 回归断言、契约 candidates 排序口径改最终返回序、接口文档待落实细节回填）
> 追加（2026-09-21 负责人验收期提出，已重新提测）：标签命中列需可见「命中了哪个标签」→ 契约 `candidates[]` / `nextRank` 增 `hitLabels: string[]`，前端 hover 展示；验收见 6a
...
- 自动判定「召回是否正确」（见开放问题）
- **覆盖范围仅三国演义**（2026-09-21 负责人确认）：诊断只针对 sango_novel_search（演义 RAG 三路召回）。风云三国是确定性 quiz 状态机、天气无检索，**均不产出诊断**；日志页对这两类工具调用显示「无检索诊断」占位
...
6. [ ] 分数表展示每条候选的排名 / chunkId / 回目 / BM25 归一化(bm25Norm) / 向量映射(cosine→(cosine+1)/2) / 标签命中 / 最终分 / 是否进注入视图 / 是否被引用；并保留原始 bm25 / 原始 cosine 供对照（要求：看着列表页即可复算 finalScore）
6a. [ ] 标签命中列可见**命中了哪个标签**：命中候选 hover 展示全部命中标签（可能多个）；未命中不展示标签（2026-09-21 负责人追加）
...
- **已落实（2026-09-21 负责人）开放问题 1 · 判定真值来源**：不做标注 UI、不接评测集。由**负责人跑若干数据并把 traceId 给 Coco**，团队据诊断数据回答验收 1~4 项 —— 真值来自负责人给定的样本集，本特性只保证「给定 traceId 看得见、算得清」
- **已落实（2026-09-21 负责人）开放问题 2 · `speaker` 去留**：先留着，后续再判断。bug-00010 已按此执行 —— 语料 schema 保留 `speaker`（不重建），工具出参不带；启用前须先修抽取规则（遗留于 bug-00005）
- **已落实（2026-09-21 负责人）死亡意图判定口径**：不要求暴露完整判定规则（触发词 / 阈值 / 排序）；只要看得见「是否生效（`detected`）」与「是否被置顶到第一条（`pinned` + 置顶后的 top1 chunkId）」即可 —— 目的是判断它有没有影响工具出参结果。契约形状已覆盖（`deathIntent.detected` / `pinned` / `chunkIds`）`n- **新增团队规定（2026-09-21 负责人）**：**禁止大批量调用 LLM**，需要调用前先向负责人报备、由负责人判断是否放行。本特性全程不调 LLM（诊断取自现有中间量），无例外；已同步根 `AGENTS.md`
...
| story-A009-02 | 接口文档：traceId `_meta` 契约 + 诊断结构 + 新表 DDL + 查询接口扩展 | 老陈 | 已交付（2026-09-21，审查回填落地口径） |
| story-A009-03 | 实现：sango 诊断产出 + 总台透传 / 落库 / 查询 + 编排侧诊断关联 | 老陈 + 小胡 | 开发完成（2026-09-21，待提测） |
| story-A009-04 | 前端：日志页检索诊断面板 | 小叶 | 开发完成（2026-09-21，待提测） |
