...
> 特性号：feat-A009
> 状态：已归档（2026-09-22 合并上线：dev-docs PR #24，mcp-server / mcp-orchestrator / mcp-web 三侧已合并）
> **验收通过（2026-09-22 负责人）：12 项验收标准全部勾选（含 6a / 6b 追加项）**
...
| 故事号 | 内容 | 负责人 | 状态 |
|--------|------|--------|------|
| story-A009-01 | 需求文档定稿与故事号 / INDEX 登记 | Coco | 已归档（2026-09-22） |
| story-A009-02 | 接口文档：traceId `_meta` 契约 + 诊断结构 + 新表 DDL + 查询接口扩展 | 老陈 | 已归档（2026-09-22） |
| story-A009-03 | 实现：sango 诊断产出 + 总台透传 / 落库 / 查询 + 编排侧诊断关联 | 老陈 + 小胡 | 已归档（2026-09-22） |
| story-A009-04 | 前端：日志页检索诊断面板（含 6a 标签明细、6b 最终分算式 tooltip 追加项） | 小叶 | 已归档（2026-09-22） |
