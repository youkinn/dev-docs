...
- 存储建议 SQLite（`better-sqlite3`，单文件零运维、分页过滤够用）；同步 API 的主线程阻塞以「写缓冲批量落盘」控制：埋点先进内存队列（如 1 秒 / 50 条一刷、单事务），单批毫秒级、可忽略（日志查询相对实时写入最多延迟约 1 秒）；不起线程、不引 worker。触发条件（写延迟明显 / 日志量大）出现时换 `sqlite3` 异步包或独立存储，另议。最终实现方案与表结构由接口文档定
...
4. 内容展示：JSON 格式化 + 语法高亮（组件 / 库小叶自选，默认 `<pre>` + 轻量着色，不强制引库）；截断处明示「（已截断）」
...
9. token 可视化（图表，位于「Token 统计」Tab）：输入 / 输出 token 双系列柱状图，按时间聚合
   ...
   - 图表库小叶自选（如 echarts / @antv/g2plot 一类轻量方案）
...
| story-A007-02 | 接口文档：埋点 / 查询 / 补报契约 + 表结构（老陈先出） | 老陈 | 待开发 |
| story-A007-03 | 实现：总台 + 编排埋点（HTTP / 队列 / LLM / 工具明细 + 存储 + 查询接口） | 老陈 + 小胡 | 待开发 |
| story-A007-04 | 前端：日志页 + traceId/时间上报与补报 + '/' 链接 + keep-alive | 小叶 | 待开发 |
