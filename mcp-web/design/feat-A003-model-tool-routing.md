...
- 端点选择条件只有一个：`mode === 'sango' && sangoService === 'random'`（store 内 computed `usesSangoRandom`）。随机一题是确定性本地命令；知识问答 / 天气 / 普通问答全部交给模型自主路由，前端不预判语义
...
- 标签保留、默认不选：用于能力可发现性与后续模板挂靠；未选时 hint「未选择时默认为普通问答」仍准确（走 /chat）；modeLabel 与输入框内标签在 mode 为 null 时返回空，展示逻辑无需额外分支
