...
- 弹框尺寸（负责人 2026-09-22 验收意见）：宽固定 960px；正文区纵向 flex 撑满视口，**不写 magic number**——`a-modal` 加 `wrap-class-name="reader-modal-wrap"`，非 scoped 样式块（选择器统一挂 `.reader-modal-wrap` 下，不外泄）覆盖 antd 默认 `top: 100px` / `padding-bottom: 24px`：`.ant-modal { top: 12px; height: calc(100vh - 24px); padding-bottom: 0 }` → `.ant-modal-content { display: flex; flex-direction: column; height: 100% }` → `.ant-modal-body { flex: 1 1 auto; min-height: 0 }` → `.reader-body { flex: 1 1 auto; min-height: 0 }`；纸面 `min-height: 100%` 随之变高；底部导航条 `flex: 0 0 auto` 常驻不随正文滚动
