...
- 翻回 / 跳转不抖动（bug-00015 修法）：`.reader-scroll` 加 `scrollbar-gutter: stable` —— 槽位常驻，loading ↔ loaded 之间滚动条出现 / 消失不再改变正文区宽度，纸面（`width: 210mm; max-width: 100%`）不横向跳。**高度链**：antd-vue 4.2.6 在 `.ant-modal` 与 `.ant-modal-content` 之间多一层无 class 包裹 div，须用 `.ant-modal > *` 一并撑开，且 `.ant-modal-content` 不能写 `height: 100%`（父级 auto 高度 → 百分比解析失败 → 整条 flex 链失效；实测 `.ant-modal` 804px 而 `.ant-modal-content` 6551px、`.reader-scroll` 6422px = scrollHeight 不可滚）；链路成立后正文区高度由视口固定（实测 `.reader-scroll` clientH 675），翻回 / 跳转置 `data = null`（正文只剩 spinner）不塌缩，正文区高度 / 纸宽 / 弹框尺寸全程不变（密集采样纸宽 794、容器高 675 恒定）
