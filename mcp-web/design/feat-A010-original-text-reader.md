...
- 翻回 / 跳转不抖动（bug-00015 修法）：`.reader-scroll` 加 `scrollbar-gutter: stable` —— 槽位常驻，loading ↔ loaded 之间滚动条出现 / 消失不再改变正文区宽度，纸面（`width: 210mm; max-width: 100%`）不横向跳；正文区高度由 flex 链路（各级 `min-height: 0` + `.reader-paper { min-height: 100% }`）撑满，翻回 / 跳转置 `data = null`（正文只剩 spinner）不塌缩，正文区高度 / 纸宽 / 弹框尺寸全程不变
- 弹框容器与页面滚动口径（bug-00017 修法）：`a-modal` 传 `:get-container="getReaderContainer"`（返回 `#app`）——antd `_util/PortalWrapper.js` 只在 portal 容器是 `document.body` 或内部 defaultContainer 时才启用滚动锁，换容器后不再向 `document.body` 注入 `overflow: hidden` / `width: calc(100% - scrollBarSize)`，页面宽度与滚动位置完全不被改写（`getOpenCount` 只在 test 环境有意义，不影响运行）；「弹框打开时页面不滚动」改由滚动链控制：`.reader-modal-wrap.ant-modal-wrap` 与 `.reader-scroll` 加 `overscroll-behavior: contain`（滚轮在遮罩上页面不动、在正文区只滚正文）
- 页面滚动口径：`html { overflow-y: auto; scrollbar-gutter: stable }` —— 槽位照旧预留（滚动条出现 / 消失不再横向抖动，bug-00011 修法效果保留），无溢出时不绘制；阅读器弹框已不依赖 antd 滚动锁（见上条 bug-00017），`html` 无需常驻绘制 inert 滚动条。已知边界：Safari 18.2 以下不支持 `scrollbar-gutter`，该情形退化为 `overflow-y: auto`，可能重现滚动条出现 / 消失的横向抖动，属已知约束，需要时另开票
- 定位时机（bug-00016 修法）：antd-vue 4.2.6 Modal **没有 `afterOpenChange`**（只有 `afterClose`），设计稿早先写的 `@after-open-change` 不存在、不要用；改为声明式触发 `watch([scrollRef, loading, data], scrollToTarget, { flush: 'post' })` —— 模板 ref 挂载（弹框正文挂载）/ `loading` 结束 / `data` 就绪（chunk 行渲染）时各测一次，满足「容器已挂载 + 目标行存在」再测量，命中则 `container.scrollTop = targetScrollTop(container.clientHeight, el)`（未传 chunkId / 目标行不在当前回 → `scrollTop = 0`，停正文顶部不报错），不做无界轮询
