...
- 翻回 / 跳转期间不清空当前正文（负责人 2026-09-22 验收意见）：`goTo()` 不再置 `data = null` —— 数据回来前保留当前回正文与标题，`loading` 期间只在正文区右上角显示绝对定位的「原文加载中…」提示（`.reader-loading-hint`，`position: absolute` 不参与布局，故不引入抖动），数据回来一次性替换（标题 / 正文 / 底部相邻回按钮同步换）；切回失败保留当前正文并 `message.error` 提示，仅首屏（`data` 为空）失败才用整块告警。实测（慢网 800ms + 逐帧采样）：loading 窗口内仍是旧回 23 行 + 标题「第 5 回」+ 提示可见，`.reader-scroll` 601 / `.ant-modal` 730 / 底部导航 665 全程不变；数据回来后一次性切到新回（15 行 / scrollTop 0），全程无空白帧（回号跳转、接口被拦失败路径同样实测）
